 ...
 * @LastEditTime: 2021-12-08 02:16:11
 */
...
struct ENode
{
    ...
    ENode *link;
    int desk; //所指向顶点的位置
    ENode(int _desk = -1, ENode *_link = nullptr, int w = 1, const Te &e = (Te)1) : edge(e, w), desk(_desk), link(_link) {}
};
...
public:
    ...
    ALGraph(const vector<vector<Tv>> &relateList, Direct dir = DIRECTED) : Graph<Tv, Te>(relateList.size(), 0, dir), VList(relateList.size())
    {
        ...
        for (int i = 0; i < this->n; i++) //头插法,注意更新边数
        {
            VList[i].vertex = Vertex<Tv>(relateList[i][0]);
            for (int j = 1; j < relateList[i].size(); j++, this->e++, VList[i].vertex.outDegree++, VList[i].vertex.inDegree++)
        ...
        for (int j = 0, v, w; j < this->e; j++)
        {
            ...
            if (v != w) //去除自反
            {
                ...
                if (!p) //去除重复
                {
                    ...
                    if (this->direct == UNDIRECTED) //无向图建立对称顶点
    ...
    virtual void remove(int v) override //返回顶点i的值，不存在返回(Tv)0
    {
        ...
        {
            remove(i, v);
            if (this->direct = DIRECTED) //有向
                remove(v, i);
        }
        VList.erase(VList.begin(), VList.begin() + 1);
        this->n--;
        ENode<Te> *p;
        for (int i = 0; i < this->n; i++)
        {

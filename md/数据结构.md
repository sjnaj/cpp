...
//算法1
 ...
 bool isValidBST(TreeNode *root)
    ...
    bool isValid(TreeNode *root, int *lower, int *upper)//lower和upper都是来源于父节点，只能用来判假，不能判真
    {
        if (!root)
            return true;
...
复杂度同基于下标优先队列的prim算法，但前者不能保证得到最短路径，而后者则不能解决带负权边的情况，如果已知最小负权值可以通过将所有权值加至正来解决。
